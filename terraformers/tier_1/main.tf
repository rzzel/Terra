...
# Security Groups
resource "aws_security_group" "t3_tier" { //change //changed
  name        = "t3_tier" //name to be changed //changed
...
resource "aws_instance" "key_ko" {//change
  ...
  subnet_id                   = aws_subnet.private_subnets["t3_tier"].id //change
  security_groups             = [aws_security_group.t3_tier.id] //change
  associate_public_ip_address = true
  //key_name                    = data.aws_key_pair.key_card //change
  key_name                    = data.aws_key_pair.key_ko.key_name //change
