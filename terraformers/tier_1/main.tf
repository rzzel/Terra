...
# Security Groups
resource "aws_security_group" "t3_tier" { 
  name        = "t3_tier" 
...
resource "aws_instance" "key_ko" {
  ami                         = "ami-0ffac3e16de16665e" 
  instance_type               = "t2.micro"
  subnet_id                   = aws_subnet.private_subnets["t3_tier"].id 
  security_groups             = [aws_security_group.t3_tier.id]
  associate_public_ip_address = true
  //key_name                    = data.aws_key_pair.key_card 
  key_name                    = data.aws_key_pair.key_ko.key_name
